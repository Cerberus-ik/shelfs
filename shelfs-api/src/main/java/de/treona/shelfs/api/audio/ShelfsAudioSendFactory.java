...
public class ShelfsAudioSendFactory implements IAudioSendFactory {
    private static final int BUFFER_DURATION = 100;
    private static final int PACKET_INTERVAL = 5;
    private static final int MAXIMUM_PACKET_SIZE = 4096;

    private final AtomicLong identifierCounter = new AtomicLong();
    private final ConcurrentHashMap.KeySetView<ShelfsAudioSendSystem, Boolean> systems = ConcurrentHashMap.newKeySet();
    ...
    public IAudioSendSystem createSendSystem(IPacketProvider packetProvider) {
        return new ShelfsAudioSendSystem(identifierCounter.incrementAndGet(), this, packetProvider);
    }

    void addInstance(ShelfsAudioSendSystem system) {
        synchronized (lock) {
            systems.add(system);
    ...
    void removeInstance(ShelfsAudioSendSystem system) {
        synchronized (lock) {
    ...
    private void populateQueues() {
        ...
        if (manager != null) {
            for (ShelfsAudioSendSystem system : systems) {
